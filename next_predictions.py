...
if __name__ == "__main__":
    ...
    # -> distribution of compliance degrees
        # PDF for prediction
    fill_color = (78/255, 154/255, 6/255, 0.7)
    line_color = '#cc0000'
    plt.figure(figsize=(8, 6))
    degree_pred = degree_vio_pred + degree_sat_pred
    if len(degree_pred) > 0:
        sns.kdeplot(degree_pred, fill=False, alpha=0.7, color=fill_color, common_norm=True, clip=(0, 1))
        plt.axvline(x=0.5, color=line_color, linestyle='--', label='Threshold')
        plt.xlabel('Degree of Compliance')
        plt.ylabel('Density')
        plt.legend()
        plt.savefig(result_path+'distribution_pred.pdf')
        plt.show()
        # PDF for ground truth
    plt.figure(figsize=(8, 6))
    degree_true = degree_vio_true + degree_sat_true 
    if len(degree_true) > 0:
        sns.kdeplot(degree_true, fill=False, alpha=0.7, color='skyblue', common_norm=True, clip=(0, 1))
        plt.axvline(x=0.5, color=line_color, linestyle='--', label='Threshold')
        plt.xlabel('Degree of Compliance')
        plt.ylabel('Density')
        plt.legend()
        plt.savefig(result_path+'distribution_true.pdf')
        plt.show()
        # scatter plot?
